    ...
    when (state.activeDialog) {
        ...
        is SettingsDialog.RecurringManage -> {
            RecurringTransactionsDialog(
                ...
                lang = lang,
                onDismiss = { viewModel?.handleIntent(SettingsIntent.DismissDialog) },
                onSaveRule = { viewModel?.handleIntent(SettingsIntent.SaveRecurringRule(it)) },
                onDeleteRule = { viewModel?.handleIntent(SettingsIntent.DeleteRecurringRule(it)) },
                onToggleRule = { rule, isEnabled -> viewModel?.handleIntent(SettingsIntent.ToggleRecurringRule(rule, isEnabled)) }
            )
        }
        is SettingsDialog.MonthlyBudget -> {
            CategoryBudgetModalDialog(
                ...
                onSave = { newBudget, newRatios ->
                    viewModel?.handleIntent(SettingsIntent.UpdateCategoryBudgets(newBudget, newRatios))
        ...
        is SettingsDialog.AboutApp -> {
            AboutAppDialog(
                onDismiss = { viewModel?.handleIntent(SettingsIntent.DismissDialog) },
                onOpenLicenses = { viewModel?.handleIntent(SettingsIntent.OpenDialog(SettingsDialog.OpenSourceLicenses)) },
                lang = lang
            )
        }
